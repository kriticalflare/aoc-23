package com.kriticalflare.part2
...
val CardTypes: HashMap<Char, Int> = hashMapOf(
    ...
    '6' to 6, '5' to 5 , '4' to 4, '3' to 3, '2' to 2 , 'J' to 1
)
...
fun getCard(input: String): Hand {
    ...
    when (cards.keys.count()) {
        ...
        2 -> {
            return  if (cards.keys.any { it == 'J' }) {
                Hand(hand = input, kind = HandType.FiveOfaKind, bid)
            } else if(cards.values.any{ it == 4 }) {
        ...
        3 -> {
            if(cards.values.any{ it == 3}) {
                if (cards.any { it.key == 'J' }) {
                    return Hand(hand = input, kind = HandType.FourOfaKind, bid)
                }
                return Hand(hand = input, kind = HandType.ThreeOfaKind, bid)
            } else {
                if (cards.any { it.key == 'J' && it.value == 2 }) {
                    return Hand(hand = input, kind = HandType.FourOfaKind, bid)
                } else if (cards.any { it.key == 'J' && it.value == 1}) {
                    return Hand(hand = input, kind = HandType.FullHouse, bid)
                }
                return Hand(hand = input, kind = HandType.TwoPair, bid)
            }
        }

        4 -> {
            return if (cards.any { it.key == 'J' }) {
                Hand(hand = input, kind = HandType.ThreeOfaKind, bid)
            }  else {
                Hand(hand = input, kind = HandType.OnePair, bid)
            }
        }
        ...
        5 -> {
            return if (cards.any { it.key == 'J'}) {
                ...
                Hand(hand = input, kind = HandType.HighCard, bid)
            }
        }
...
fun SolvePart2(input: String): Int{
    return readInput(input).map { getCard(it)
